...
arfa1=[];
EnergyEfficiency_NOMA_DC = zeros(1,length(BS_Radius));
EnergyEfficiency2 = zeros(1,length(BS_Radius));
  for Num_R=1:length(BS_Radius)
   BS_Radius1=BS_Radius(Num_R);
 ...
 BS.location_x = 0;  
        BS.location_y = 0;        
        [x,y]=DrawPoint( BS.location_x,BS.location_y,BS_Radius1,UE_Num);
 for j=1:UE_Num
         UE(j).location_x=x(j);
        UE(j).location_y=y(j);
         UE(j).location_x;
 ...
 h=[fencu1;fencu2;fencu3;fencu4;fencu5;fencu1;fencu2;fencu3;fencu4;fencu5;];
  h1=sort(h,2);
   ...
   for index=1:1:length(M_array)
    ...
    EE;
   EnergyEfficiency2(Num_R)= EnergyEfficiency2(Num_R)+ sum(EE);
   arfa1;
  arfa=[rot90(arfa1)];
%arfa1=[ropt(M)];
%arfa=[rot90(ropt(M))];
 SubChannel_MatchGain1=zeros(ZXD,1);
   ...
   EnergyEfficiency_NOMA_DC(Num_R)= EnergyEfficiency_NOMA_DC(Num_R)+EnergyEfficiency.*B1;
  end
 ...
 %  EnergyEfficiency_NOMA_DC(Num_BS_Power)=     EnergyEfficiency_NOMA_DC(Num_BS_Power);
  ...
  EnergyEfficiency_NOMA_DC=EnergyEfficiency_NOMA_DC./(Iter_MonteCarlo_Max);
   EnergyEfficiency2= EnergyEfficiency2/(Iter_MonteCarlo_Max);
BS_Radius = 100:100:1000; 

plot(BS_Radius, EnergyEfficiency_NOMA_DC,'b-*',BS_Radius, EnergyEfficiency2,'r-o');
grid on;
legend('DC算法','等功率分配算法')
ylabel('系统能效(bits/Joule)');
xlabel('基站的发射功率(watt)');
