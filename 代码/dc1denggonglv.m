...
EnergyEfficiency2 = zeros(1,length(Power));
 ...
 h=[fencu1;fencu2;fencu3;fencu4;fencu5;fencu1;fencu2;fencu3;fencu4;fencu5;];
  h1=sort(h,2);
   ...
   for index=1:1:length(M_array)
    ...
    EE;
   EnergyEfficiency2(Num_BS_Power)=EnergyEfficiency2(Num_BS_Power)+ sum(EE);
   arfa1;
  arfa=[rot90(arfa1)];
%arfa1=[ropt(M)];
%arfa=[rot90(ropt(M))];
...
EnergyEfficiency_NOMA_DC=[0,EnergyEfficiency_NOMA_DC];
  EnergyEfficiency2=[0,EnergyEfficiency2];
  
plot(Power, EnergyEfficiency_NOMA_DC,'b-*',Power, EnergyEfficiency2,'r-o');
grid on;
legend('DC算法','等功率分配算法')
