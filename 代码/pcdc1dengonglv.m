...
arfa1=[];
EnergyEfficiency_NOMA_DC = zeros(1,length(Pc_range));
EnergyEfficiency2 = zeros(1,length(Pc_range));
for  Num_Pc=1:length(Pc_range)
  Pc = Pc_range(Num_Pc);
 ...
 h=[fencu1;fencu2;fencu3;fencu4;fencu5;fencu1;fencu2;fencu3;fencu4;fencu5;];
  h1=sort(h,2);
   ...
   for index=1:1:length(M_array)
    ...
    EE;
   EnergyEfficiency2(Num_Pc)=EnergyEfficiency2(Num_Pc)+ sum(EE);
   arfa1;
  arfa=[rot90(arfa1)];
%arfa1=[ropt(M)];
%arfa=[rot90(ropt(M))];
 SubChannel_MatchGain1=zeros(ZXD,1);
   ...
   EnergyEfficiency_NOMA_DC(Num_Pc)= EnergyEfficiency_NOMA_DC(Num_Pc)+EnergyEfficiency.*B1;
  end
...
plot(Pc_range, EnergyEfficiency_NOMA_DC,'b-*',Pc_range, EnergyEfficiency2,'r-o');
grid on;
legend('DC算法','等功率分配算法')
